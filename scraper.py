...
# A list of genres hosted on wikiart.org as well as the number of pages to pull images from, numbers were set from
# manual inspection and are only approximations of how many pages each genre contains
...
# Access the html of the page given a genre and pagenumber that are used to generate a url, from this html find the
# urls of all images hosted on the page using page layout as of June 2017, return a list of alls urls to paintings
def soupit(j, genre):
    try:
...
# Given a url for an image, we download and save the image while also recovering information about the painting in
# the saved name depending on the length of the file.split('/') information (which corresponds to how much
...
def dwnld(web, genre):
    ...
    # If we get an exception in this operation it is probably because there was a nonstandard unicode character in
    # the name of the painting, do some fancy magic to fix this in the exception handling code
    try:
...
# We can run both the url retrieving code and the image downloading code in parallel, and we set up the logic for
# that here
